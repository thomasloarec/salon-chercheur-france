...
serve(async (req) => {
  ...
  try {
    ...
    if (!spreadsheetId1 || !spreadsheetId2) {
      throw new Error('Spreadsheet IDs are required');
    }

    console.log(`Importing from spreadsheets: ${spreadsheetId1} and ${spreadsheetId2}`);

    // Import events from All_Evenements sheet
    ...
    if (!eventsResponse.ok) {
      throw new Error(`Failed to fetch events data: ${eventsResponse.statusText}`);
    }
    ...
    if (!eventsRows || eventsRows.length < 2) {
      throw new Error('No events data found or invalid format');
    }
    ...
    // Import exposants from E46 sheet
    console.log('Fetching exposants data...');
    ...
    if (!exposantsResponse.ok) {
      throw new Error(`Failed to fetch exposants data: ${exposantsResponse.statusText}`);
    }
    ...
    if (!exposantsRows || exposantsRows.length < 2) {
      console.log('No exposants data found, skipping...');
    } else {
      // Get headers and map data
    ...
    const summary = {
      ...
      exposantsImported: exposantsRows?.length > 1 ? Math.max(0, exposantsRows.length - 1) : 0,
