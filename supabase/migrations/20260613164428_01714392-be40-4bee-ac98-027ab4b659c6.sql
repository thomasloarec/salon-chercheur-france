...
BEGIN
  ...
  IF v_campaign_id IS NOT NULL THEN
    ...
    SET claim_status   = 'claimed',
        ...
        next_send_at   = CASE WHEN novelty_step = 0 AND novelty_status NOT IN ('published','declined') THEN now() + interval '1 day' ELSE next_send_at END,
        updated_at     = now()
  ...
  SET claim_status   = 'claimed',
      ...
      next_send_at   = CASE WHEN oc.novelty_step = 0 AND oc.novelty_status NOT IN ('published','declined') THEN now() + interval '1 day' ELSE oc.next_send_at END,
      updated_at     = now()
