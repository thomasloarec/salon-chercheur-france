-- Suppression de l'ancienne fonction search_events
DROP FUNCTION IF EXISTS public.search_events(uuid[], text[], integer[], text[], integer, integer);

-- Création de la nouvelle fonction search_events avec UUID en sortie
...
RETURNS TABLE(
  id uuid,  -- ⭐ NOUVEAU : ajout de l'UUID comme premier champ
  id_event text, 
  nom_event text, 
  date_debut date, 
  date_fin date, 
