-- =====================================================================
-- get_radar_salon_missions etendu pour inclure les rencontres manuelles.
-- Applique en prod le 16/08/2026, verifie (rencontre remonte, CRM non regresse).
-- A committer apres 20260816130000_radar_terrain_encounter.sql.
-- =====================================================================
...
declare
  ...
  v_event jsonb; v_companies jsonb; v_manual jsonb;
  v_member_count int;
begin
    ...
    select jsonb_build_object(
      ...
      'notes', case when ms.id is null then '[]'::jsonb else coalesce(
        (select jsonb_agg(jsonb_build_object(
            'id', n.id, 'body', n.body, 'source', n.source, 'created_at', n.created_at,
            'created_by', n.created_by,
            'author_name', public.radar_member_display_name(n.created_by)
      ...
      'tasks', case when ms.id is null then '[]'::jsonb else coalesce(
        (select jsonb_agg(jsonb_build_object(
            'id', t.id, 'body', t.body, 'due_at', t.due_at, 'done', t.done, 'source', t.source,
            'created_at', t.created_at,
            'created_by', t.created_by,
            'author_name', public.radar_member_display_name(t.created_by)
  ...
  select coalesce(jsonb_agg(comp order by comp->>'company_name'), '[]'::jsonb) into v_manual
  from (
    select jsonb_build_object(
      'crm_company_id', null,
      'mission_id', ms.id,
      'company_name', coalesce(ms.manual_company_name, 'Entreprise rencontrée'),
      'nom_exposant', ms.manual_company_name,
      'website', null, 'normalized_domain', null, 'description', null,
      'id_exposant', null, 'stands', '[]'::jsonb,
      'needs_review', null, 'name_similarity', null,
      'crm_status_raw', null, 'pref_status', null, 'relationship_status', null,
      'objective', ms.objective, 'opening_line', ms.opening_line,
      'top_q1', ms.top_q1, 'top_q2', ms.top_q2, 'top_q3', ms.top_q3,
      'origin', ms.origin,
      'ai_meta', ms.ai_meta, 'ai_generated_at', ms.ai_generated_at,
      'ai_field_sources', coalesce(ms.ai_field_sources, '{}'::jsonb),
      'visited', coalesce(ms.visited, false), 'visited_at', ms.visited_at,
      'notes', coalesce(
        (select jsonb_agg(jsonb_build_object(
            'id', n.id, 'body', n.body, 'source', n.source, 'created_at', n.created_at,
            'created_by', n.created_by, 'author_name', public.radar_member_display_name(n.created_by)
         ) order by n.created_at desc)
         from public.radar_mission_notes n where n.mission_id = ms.id), '[]'::jsonb),
      'tasks', coalesce(
        (select jsonb_agg(jsonb_build_object(
            'id', t.id, 'body', t.body, 'due_at', t.due_at, 'done', t.done, 'source', t.source,
         ...
         from public.radar_mission_tasks t where t.mission_id = ms.id), '[]'::jsonb)
    ) as comp
    from public.radar_missions ms
    where ms.radar_account_id = v_account_id
      and ms.event_id = p_event_id
      and ms.crm_company_id is null
      and ms.origin = 'rencontre'
  ) subm;

  return jsonb_build_object(
    'event', v_event,
    'active_member_count', v_member_count,
    'companies', coalesce(v_companies, '[]'::jsonb) || coalesce(v_manual, '[]'::jsonb)
  );
end; $function$;
