...
-- Fix the search_events RPC function to properly use event_sectors table with UUIDs
...
DECLARE
  ...
  total_events bigint;
  sector_events_count integer;
BEGIN
  -- Conditions de base avec les bons noms de colonnes
  ...
  -- Filtres secteurs via la table de liaison event_sectors
  IF array_length(sector_ids, 1) > 0 THEN
    -- Log pour debug
    SELECT COUNT(*) INTO sector_events_count 
    FROM event_sectors 
    WHERE sector_id = ANY(sector_ids);
    
    RAISE NOTICE 'RPC search_events: % lignes dans event_sectors pour les secteur IDs: %', sector_events_count, sector_ids;
  ...
  RAISE NOTICE 'RPC search_events: % événements trouvés au total', total_events;
  
  -- Retourner les résultats paginés avec le total (utilise les bons noms de colonnes)
