...
SELECT oc.id,
  ...
  WHERE oc.claim_status = 'claimed'::text AND oc.novelty_status = 'active'::text AND oc.opt_out = false AND oc.novelty_step < 3 AND (oc.next_send_at IS NULL OR oc.next_send_at <= now()) AND e.date_debut >= CURRENT_DATE AND e.visible = true AND e.is_test = false AND NOT (EXISTS ( SELECT 1
           FROM novelties n
