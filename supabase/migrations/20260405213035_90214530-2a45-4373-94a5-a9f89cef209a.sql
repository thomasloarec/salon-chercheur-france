...
-- Fix search_path for related_events function
CREATE OR REPLACE FUNCTION public.related_events(p_event_id text, p_limit integer DEFAULT 6)
 RETURNS TABLE(id uuid, id_event text, slug text, nom_event text, date_debut date, date_fin date, url_image text, nom_lieu text, ville text, sectors uuid[], shared_sectors_count integer)
 LANGUAGE plpgsql
 STABLE
