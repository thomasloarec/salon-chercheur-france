...
-- Migration: Rename columns from Google Sheets format to Airtable format
-- Date: 2025-07-18

-- First, rename columns according to the mapping
ALTER TABLE public.events RENAME COLUMN name TO name_event;
...
-- Keep venue_name and rename it to nom_lieu (will drop location after audit)
ALTER TABLE public.events RENAME COLUMN venue_name TO nom_lieu;

-- Drop unused columns after audit confirms they're not referenced
DROP COLUMN IF EXISTS public.events.location;
DROP COLUMN IF EXISTS public.events.event_url;
DROP COLUMN IF EXISTS public.events.organizer_name;
DROP COLUMN IF EXISTS public.events.organizer_contact;
...
BEGIN
  ...
  RETURN QUERY EXECUTE format('
    SELECT e.id, e.name_event, e.description_event, e.date_debut, e.date_fin, e.secteur, e.ville, e.region, e.country,
...
-- Update triggers that reference old column names
...
BEGIN
    ...
    IF NEW.slug IS NULL THEN
        -- Extract year from date_debut (new column name)
        event_year := EXTRACT(YEAR FROM NEW.date_debut);
        
        -- Generate slug using new column names
        new_slug := generate_event_slug(NEW.name_event, NEW.ville, event_year);
        ...
        WHILE EXISTS (SELECT 1 FROM events WHERE slug = new_slug AND id != COALESCE(NEW.id, '00000000-0000-0000-0000-000000000000'::uuid)) LOOP
            counter := counter + 1;
            new_slug := generate_event_slug(NEW.name_event, NEW.ville, event_year) || '-' || counter;
        END LOOP;
