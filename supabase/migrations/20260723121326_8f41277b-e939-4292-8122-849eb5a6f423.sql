-- LOT 1 — Index sémantique des Nouveautés

CREATE TABLE IF NOT EXISTS public.novelty_embeddings (
  novelty_id  uuid PRIMARY KEY REFERENCES public.novelties(id) ON DELETE CASCADE,
  embedding   vector(1024) NOT NULL,
  embedded_at timestamptz NOT NULL DEFAULT now()
);

CREATE INDEX IF NOT EXISTS idx_novelty_embeddings_hnsw
  ON public.novelty_embeddings
  USING hnsw (embedding vector_cosine_ops);

ALTER TABLE public.novelty_embeddings ENABLE ROW LEVEL SECURITY;

COMMENT ON TABLE public.novelty_embeddings IS
  'Index sémantique des Nouveautés publiées. Alimenté par embed_pending_novelties(). Une Nouveauté dépubliée voit son embedding supprimé.';
...
BEGIN
  ...
  LOOP
    ...
    FROM (
      SELECT n.id,
        trim(
          coalesce(n.title, '') || ' ' ||
          CASE n.type
            WHEN 'Launch'        THEN 'lancement nouveau produit'
            WHEN 'Update'        THEN 'mise a jour evolution version'
            WHEN 'Demo'          THEN 'demonstration essai sur stand'
            WHEN 'Special_Offer' THEN 'offre speciale promotion'
            WHEN 'Partnership'   THEN 'partenariat collaboration'
            WHEN 'Innovation'    THEN 'innovation technologie'
            ELSE ''
          END || ' ' ||
          coalesce(n.summary, '')  || ' ' ||
          coalesce(n.reason_1, '') || ' ' ||
    ...
    FOR i IN 1 .. array_length(v_ids, 1) LOOP
      ...
      VALUES (v_ids[i], ((v_emb -> (i - 1))::text)::vector, now())
...
GRANT EXECUTE ON FUNCTION public.match_novelties_semantic(text, uuid, boolean, double precision, integer)
  TO anon, authenticated, service_role;

COMMENT ON FUNCTION public.match_novelties_semantic IS
  'Recherche sémantique sur les Nouveautés publiées. Renvoie des lignes individuelles uniquement : aucun agrégat, aucune donnée commerciale.';

SELECT cron.schedule(
  'embed-pending-novelties',
  '*/30 * * * *',
  $cron$ SELECT public.embed_pending_novelties(4); $cron$
);
