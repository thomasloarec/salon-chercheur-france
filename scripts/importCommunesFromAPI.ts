...
if (!SUPABASE_SERVICE_KEY) {
  ...
  console.log('SUPABASE_SERVICE_KEY=<your_service_role_key> npx ts-node scripts/importCommunesFromAPI.ts');
  process.exit(1);
}
...
async function fetchCommunesFromAPI(): Promise<APICommune[]> {
  ...
  try {
    ...
    const data: APICommune[] = await response.json();
...
function transformData(apiData: APICommune[]): CommuneRow[] {
...
async function insertInBatches(rows: CommuneRow[]): Promise<void> {
