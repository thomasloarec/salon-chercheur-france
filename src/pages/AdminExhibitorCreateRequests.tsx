...
import { Search, Check, X, Plus, Building, ExternalLink } from 'lucide-react';
...
export default function AdminExhibitorCreateRequests() {
  const [searchQuery, setSearchQuery] = useState('');
  const debouncedSearch = useDebounce(searchQuery, 300);
  const { toast } = useToast();
  const queryClient = useQueryClient();

  // Fetch create requests
  const { data: requests, isLoading, error } = useQuery({
    queryKey: ['admin-exhibitor-create-requests', debouncedSearch],
    queryFn: async (): Promise<CreateRequest[]> => {
      let query = supabase
      ...
      if (debouncedSearch) {
        query = query.ilike('proposed_name', `%${debouncedSearch}%`);
      }
  ...
  const updateRequestMutation = useMutation({
    mutationFn: async ({ requestId, action }: { requestId: string; action: 'approve' | 'reject' }) => {
      const { error } = await supabase.functions.invoke('exhibitors-manage', {
        body: {
          action: action === 'approve' ? 'approve_create' : 'reject_create',
    ...
    onSuccess: (_, { action }) => {
      toast({
        ...
        description: action === 'approve' 
          ? 'L\'exposant a été créé et l\'utilisateur en est maintenant propriétaire.' 
          : 'La demande a été rejetée.',
      });
      queryClient.invalidateQueries({ queryKey: ['admin-exhibitor-create-requests'] });
    },
    onError: (error) => {
                  ...
                  {requests?.map((request) => (
                    <div key={request.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-muted/50 transition-colors">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h3 className="font-medium">{request.proposed_name}</h3>
