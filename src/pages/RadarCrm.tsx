...
const RadarCrmPage: React.FC = () => {
  ...
  const hubspotConnection = useMemo(
    () => connectionsData.find((c) => c.provider === 'hubspot' && c.status === 'active'),
    [connectionsData]
  );
          ...
          <div className="rounded-2xl border border-border bg-card p-4 md:p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="space-y-1 min-w-0">
              <p className="text-sm font-medium text-foreground flex items-center gap-2">
                ...
                {hubspotConnection ? 'HubSpot connecté' : 'Connecter HubSpot'}
              </p>
              <p className="text-sm text-muted-foreground">
                {hubspotConnection
                  ? `Connecté au portail ${hubspotConnection.portal_id}`
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto">
              {hubspotError && !crmConnectionsLoading && (
