...
const RechercheIA = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // onAuthStateChange (AuthContext) mettra à jour `session` ; authReady suivra.
      setAuthReady(true);
    })();
  }, [authLoading, session]);
  ...
  const send = async (raw: string) => {
    ...
    setInput('');
    setAsking(true);
    // Un mur "mou" affiché précédemment est levé dès qu'une nouvelle requête part.
    ...
    try {
      ...
      // Mur dur : pas de réponse, aucun crédit disponible.
      ...
      // Mur mou : dernier crédit consommé → CTA préparé sous la réponse.
      if (data?.wall?.soft) {
  ...
  const handleUpgraded = () => {
    // Compte converti : crédits réinitialisés côté serveur (6 au total).
    setWall(null);
    setCredits(null);
  ...
  const handlePaidIntent = async () => {
    ...
    try {
      ...
      /* silencieux : la V1 mime le paiement, l'échec de log ne bloque pas l'UX */
    }
  };
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 w-full px-6 mx-auto max-w-3xl py-8 flex flex-col">
        {/* Hero — visible en permanence, compact une fois la conversation lancée */}
        <section className={`section-rule ${hasStarted ? 'mb-6' : 'mb-8'}`}>
          ...
          <h1 className={`heading-display text-foreground ${hasStarted ? 'text-2xl md:text-3xl' : 'text-3xl md:text-5xl'}`}>
        ...
        {/* Barre de saisie */}
        <form onSubmit={onSubmit} className="mt-4 sticky bottom-4">
